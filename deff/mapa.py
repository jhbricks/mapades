...
def mx_mn (area,arq,ind,calc,tipo=None,unidade=None) :
    ...
    st.markdown("<h3><font size='+5'> Municípios com o <font size='+5' color='#3f007d'>maior</font> e <font size='+5' color='#ba2db4'>menor</font> <font size='+5'> valor:</font></h3>", unsafe_allow_html=True)
    #st.markdown(f"<p style='line-height: 0.7;'><font size='+10' color='#3f007d'>{arrow_u}</font> <font size='+5'>{max_str} = {ind_mx}</font></p>", unsafe_allow_html=True)
    ...
    if unidade:
      st.markdown(f"""<p style='line-height: 0.7;'><font size='+10' color='#3f007d'>{arrow_u}</font> <font size='+5'>{max_str} = {ind_mx} {unidade}</font>  
