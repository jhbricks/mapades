...
def mx_mn (area,arq,ind,unidade=None):
    ...
    if unidade is not None:
        st.markdown(f"""<p><font size='+7' color='darkpurple'>{arrow_u}</font> <font size='+5'>{max_str} = {ind_mx} {unidade}</font>  
        <font size='+7' color='purple'>{arrow_d}</font> <font size='+5'>{min_str} = {ind_mn} {unidade}</font></p>""", unsafe_allow_html=True)
    else:
